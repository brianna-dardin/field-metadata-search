...
class Object:
    ...
    def get_fields(self, token, instance, api_version):
        ...
        request_url = instance + '/services/data/v'+str(api_version)+'/tooling/query?q='
        headers = {'Authorization' : 'OAuth ' + token}
        
        if self.standard:
            where_obj = self.sobject_name
        else:
            no_suffix = self.sobject_name.replace('__c','').replace('__x','')\
                            .replace('__mdt','')
            
            object_query = 'SELECT Id FROM CustomObject WHERE DeveloperName = \''\
                            + no_suffix + '\''
        ...
        field_query = 'SELECT NamespacePrefix,DeveloperName,CreatedDate,LastModifiedDate '\
                    + 'FROM CustomField WHERE TableEnumOrId=\''+where_obj+'\''
    ...
    def search_fields(self):
        ...
        no_search_obj = ['aura','classes','components','duplicateRules','email',
                         'approvalProcesses','flows','layouts','pages','triggers']
        search_obj_list = ['assignmentRules','autoResponseRules',
                           'escalationRules','matchingRules','objects',
                           'sharingRules','workflows']
